...
void test_sudo_action_routes_to_sudo_vector() {
    auto tokens = lex_line("@allow-sudo \"/etc/hosts\" >> \"hosts\"");
    CHECK(!tokens.empty() && tokens[0].type == Token::ACTION, "sudo action: lexer emits ACTION token");
    CHECK(!tokens.empty() && tokens[0].name == "allow-sudo", "sudo action: ACTION token carries 'allow-sudo'");
...
void test_sudo_action_link_variant() {
    auto tokens = lex_line("@allow-sudo \"/etc/passwd\" -> \"passwd\"");
    DotlangParser p;
    p.feed((tokens));
    p.parseMain();
...
void test_sudo_directive_alone_does_not_imply_action() {
    ...
    // an explicit `@allow-sudo` action token on a path line, that line
    ...
    CHECK(p.sudo_copy_files.empty(), "directive alone: sudo_copy_files stays empty without @allow-sudo action");
}
...
void test_sudo_state_resets_between_parseMain_calls() {
    ...
    auto tokens1 = lex_line("@allow-sudo \"/etc/hosts\" >> \"hosts\"");
    p.feed((tokens1));
    p.parseMain();
    CHECK(p.sudo_copy_files.size() == 1, "reset check: first parseMain populates sudo_copy_files");

    // second parseMain call, no @allow-sudo this time -- must NOT leak sudo state
